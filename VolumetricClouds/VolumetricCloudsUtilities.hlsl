...
// ═══════════════════════════════════════════════════════════════
//  TORUS CLOUD VOLUME GLOBALS
//  _TorusMajorRadius : radius of the ring (e.g. 3000)
//  _TorusMinorRadius : radius of the tube  (e.g. 600)
//  _TorusAngle       : travel angle in radians
//  _TorusCenter      : world center of the torus
//  _TorusRotation    : float3 Euler rotation (radians) of the torus orientation
// ═══════════════════════════════════════════════════════════════
float  _TorusMajorRadius;
float  _TorusMinorRadius;
float  _TorusAngle;
float3 _TorusCenter;
float3 _TorusRotation; // Euler XYZ in radians

// Rotate a point by Euler XYZ angles
float3 RotateEuler(float3 p, float3 euler)
{
    // Rotate X
    float cx = cos(euler.x), sx = sin(euler.x);
    float3 p1 = float3(p.x, cx*p.y - sx*p.z, sx*p.y + cx*p.z);
    // Rotate Y
    float cy = cos(euler.y), sy = sin(euler.y);
    float3 p2 = float3(cy*p1.x + sy*p1.z, p1.y, -sy*p1.x + cy*p1.z);
    // Rotate Z
    float cz = cos(euler.z), sz = sin(euler.z);
    return float3(cz*p2.x - sz*p2.y, sz*p2.x + cz*p2.y, p2.z);
}

// Inverse rotation (apply negative Euler in reverse order)
float3 RotateEulerInverse(float3 p, float3 euler)
{
    return RotateEuler(p, -euler.zyx);
}

// Torus implicit in LOCAL torus space (ring lies in XZ plane)
float TorusImplicit(float3 p, float R, float r)
{
    // Apply inverse torus rotation first
    float3 local = RotateEulerInverse(p, _TorusRotation);
    float xzLen = length(local.xz);
    float q = xzLen - R;
    return q * q + local.y * local.y - r * r;
}

// Remap world-space sample position into planet-space for cloud density lookup.
float3 TorusToSpherePS(float3 relPosWS)
{
    float R = _TorusMajorRadius;

    // Transform into torus local space (undo torus orientation)
    float3 local = RotateEulerInverse(relPosWS, _TorusRotation);

    // Find nearest point on major circle (in local XZ plane)
    float xzLen = length(local.xz);
    float2 ringDir = (xzLen > 0.001) ? local.xz / xzLen : float2(1.0, 0.0);
    float3 tubeCenter = float3(ringDir.x * R, 0.0, ringDir.y * R);

    // Vector from tube center to sample
    float3 toSample = local - tubeCenter;

    // Map to cloud altitude space
    float3 psPos;
    psPos.y  = toSample.y + _LowestCloudAltitude + (_HighestCloudAltitude - _LowestCloudAltitude) * 0.5;
    psPos.xz = tubeCenter.xz * 0.01 + toSample.xz;
    return psPos;
}
// ═══════════════════════════════════════════════════════════════
...
{
    // TORUS MODE: camera is inside tube, march through tube diameter
    if (_TorusMajorRadius > 0.5)
    {
        ZERO_INITIALIZE(RayMarchRange, rayMarchRange);
        float3 relOrigin = originWS - _TorusCenter;

        // March forward to find entry/exit of torus tube
        // We step along the ray and find sign changes of the torus implicit
        float R = _TorusMajorRadius;
        float r = _TorusMinorRadius;
        float maxDist = (R + r) * 3.0; // generous upper bound
        float stepSize = maxDist / 128.0;

        float tEntry = -1.0;
        float tExit  = -1.0;
        bool  inside = TorusImplicit(relOrigin, R, r) < 0.0;

        if (inside)
        {
            // Camera is inside tube — start at 0, find exit
            tEntry = 0.0;
            for (int i = 1; i < 128; i++)
            {
                float t = float(i) * stepSize;
                float3 p = relOrigin + dir * t;
                if (TorusImplicit(p, R, r) >= 0.0) { tExit = t; break; }
            }
            if (tExit < 0.0) tExit = maxDist;
        }
        else
        {
            // Camera is outside — find entry then exit
            for (int i = 0; i < 128; i++)
            {
                float t = float(i) * stepSize;
                float3 p = relOrigin + dir * t;
                if (TorusImplicit(p, R, r) < 0.0)
                {
                    tEntry = t;
                    // find exit
                    for (int j = i+1; j < 128; j++)
                    {
                        float t2 = float(j) * stepSize;
                        float3 p2 = relOrigin + dir * t2;
                        if (TorusImplicit(p2, R, r) >= 0.0) { tExit = t2; break; }
                    }
                    if (tExit < 0.0) tExit = maxDist;
                    break;
                }
        ...
        if (tEntry < 0.0) return false; // no intersection

        rayMarchRange.start = tEntry;
        rayMarchRange.end   = tExit;
        return true;
    }
